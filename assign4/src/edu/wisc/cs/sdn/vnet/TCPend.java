...
/**
 * TCP implementation for the SDN Virtual Network
 */
public class TCPend {

    /**
     * Server mode - receive data and write to file
     * @param sPort the port to listen on
     * @param filename the file to write data to
     * @param mtu the maximum transmission unit
     * @param sws the sliding window size
     */
    public static void server(int sPort, String filename, int mtu, int sws) {
        TCPServer server = new TCPServer(sPort, mtu, sws, filename);
        server.run();
    }

    /**
     * Client mode - read file and send data
    ...
    /**
     * Main method to parse command line arguments and start the appropriate mode
    ...
    public static void main(String[] args) {
        ...
        if (hostPort == -1 || mtu <= 0 || filename == null || sws <= 0) {
            System.out.println("Missing or invalid required arguments");
            printUsage();
            System.exit(1);
        }
        
        // Determine if we're running in client or server mode
        if (serverIP != null && serverPort > 0) {
            // Client mode
            System.out.println("Running in client mode");
